...
<?php 
 if(isset($_POST['submit']))
		 ...
		 $status=$_POST['status'];
		 $sql="update tbl_paymenthistory set order_status=$status,accepted_status=1 where payment_id=$payment_id";
		 if(mysqli_query($sql,$connection_1))
				{
...
$sql="select * from tbl_paymenthistory where order_status=1";
...
<table>
 ...
 <td width="30%"><?php echo date('j - M',strtotime($rec['created_date'])); ?></td>
    ...
    <tr class="tr-border">
      ...
      <td class="table-label products-line" colspan="2"><?php echo $rec['address']; ?>.</td>
...
<td class="table-label">Total Rs. <?php echo $rec['amount']; ?></td>
    ...
    <option <?php if($rec['order_status']==2){ echo 'selected="selected"';} ?> value="2">Delivered</option>
     ...
     <input type="hidden" name="payment_id" value="<?php echo $rec['payment_id']; ?>" />
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js"></script>
	   ...
	   {
       $(function(){
       $("#list1").load("Purchasedintransitorders.php");
