...
<?php 
    $id=$_GET['id'];
    $sql="update tbl_paymenthistory set accepted_status=1 where payment_id=$id";
    $query=mysqli_query($connection_1,$sql);
    $sql="select * from tbl_paymenthistory where payment_id=$id";
    $query=mysqli_query($connection_1,$sql);
...
<tr class="tr-border">
       <td class="table-label" colspan="2" ><?php echo $rec['address']; ?>.</td>
